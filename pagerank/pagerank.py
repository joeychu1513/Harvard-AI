...
def iterate_pagerank(corpus, damping_factor):
    ...
    return new_proba_pages
            ...
            # # if no link at all
            # if formula_latter == 0:
            #     for no_link_page in old_proba_pages:
            #         formula_latter += damping_factor * old_proba_pages[no_link_page] / len(list_all_pages)

            # if len(corpus[old_proba_page]) == 0:
            #     for page in old_proba_pages:
            #         if len(corpus[page]) == 0:
            #             # # check if there is new probabily, if so, use it first
            #             # if page in new_proba_pages:
            #             #     formula_latter += damping_factor * new_proba_pages[page] / len(list_all_pages)
            #             # else:
            #             formula_latter += damping_factor * old_proba_pages[page] / len(list_all_pages)
            #         else:
            #             formula_latter += damping_factor * old_proba_pages[page] / len(corpus[page])
            # else:
            #     for page in corpus[old_proba_page]:
            #         # # check if there is new probabily, if so, use it first
            #         # if page in new_proba_pages:
            #         #     formula_latter += damping_factor * new_proba_pages[page] / len(corpus[old_proba_page])
            #         # else:
            #         if len(corpus[page]) == 0:
            #             formula_latter += damping_factor * old_proba_pages[page] / len(list_all_pages)
            #         else:
